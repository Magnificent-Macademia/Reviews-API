...
module.exports = {
  getReviews: (params, cb) => {
    ...
    const queryStr = `select reviews.name as reviewer_name, reviews.*, array_agg(photos.url) as photos from reviews inner join photos on reviews.review_id=photos.review_id where reviews.product_id=${params.product_id} GROUP BY reviews.review_id ${params.sort ? sortMethod[params.sort] : ''} limit ${params.count ? params.count : 5};`;
